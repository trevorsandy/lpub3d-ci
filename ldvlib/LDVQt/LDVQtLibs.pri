...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    if (unix:!BUILD_TINYXML:exists(/usr/include/tinyxml.h)|exists($${SYSTEM_PREFIX_}/include/tinyxml.h)) {
    ...
    if (unix:!BUILD_GL2PS:exists(/usr/include/gl2ps.h)|exists($${SYSTEM_PREFIX_}/include/gl2ps.h)) {
    ...
    if (unix:!BUILD_3DS:exists(/usr/include/lib3ds.h)|exists($${SYSTEM_PREFIX_}/include/lib3ds.h)) {
    ...
    if (unix:!BUILD_MINIZIP:exists(/usr/include/minizip/unzip.h)|exists($${SYSTEM_PREFIX_}/include/minizip/unzip.h)) {
    ...
    if (unix:!BUILD_PNG:exists(/usr/include/png.h)|exists($${SYSTEM_PREFIX_}/include/png.h)) {
    ...
    if (unix:!BUILD_JPEG:exists(/usr/include/jpeglib.h)|exists($${SYSTEM_PREFIX_}/include/jpeglib.h)) {
...
contains(LOAD_LDV_LIBS,True) {
    ...
    win32-msvc*:CONFIG(debug, debug|release):!BUILD_LDV_LIBS {
        equals(VER_USE_LDVIEW_DEV,True) {
    ...
    BUILD_LDV_LIBS {
        ...
        LIBS            += -L$${LDVLIBDIR} -L$${LDV3RDLIBDIR}
    }
...
#    message("~~~ $${LPUB3D} Library path: $$LDVLIBRARY ~~~ ")
    ...
    BUILD_ZLIB {
        LIBS               += $$ZLIB_SRC
    } else:!contains(USE_LOCAL_ZLIB_LIB,False) {
        LIBS               += $$ZLIB_LIB
    }
