...
CONFIG(debug, debug|release) {
    ...
    BUILD_LDV_LIBS {
        VER_LDVIEW_DEV_REPOSITORY = LDView
...
BUILD_LDV_LIBS {
    ...
    LIB_LDVIEW  = TCFoundation$${POSTFIX}.lib
    LIB_LDVIEW_PATH = LDView/TCFoundation/$${DESTDIR}/$${LIB_LDVIEW}
    exists($${LIB_LDVIEW_PATH}) {
        LOAD_LDV_HEADERS = True
    } else {
        message("~~~ FATAL_ LDVIEW IS NOT AVAILABLE - CANNOT LOAD HEADERS ~~~ ")
        LOAD_LDV_HEADERS = False
    }
} else {
    LOAD_LDV_HEADERS = True
}
