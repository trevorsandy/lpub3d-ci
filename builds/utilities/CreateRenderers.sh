...
# Set build type
[ -z "${LP3D_BUILD_TYPE}" ] && LP3D_BUILD_TYPE="Release" || true
tempvar="$(echo ${LP3D_BUILD_TYPE} | awk '{print tolower($0)}')"
[[ "$tempvar" != "release" && "$tempvar" != "debug" ]] && \
Info "Build Type...............[Invalid value: ${LP3D_BUILD_TYPE}, using 'Release']" && \
tempvar="release" || \
Info "Build Type...............[${LP3D_BUILD_TYPE}]"
LP3D_BUILD_TYPE="${tempvar}" && unset tempvar
...
if [[ "$TARGET_CPU" = "x86_64" || "$TARGET_CPU" = "aarch64" || "$TARGET_CPU" = "arm64" ]]; then
  BUILD_ARCH="64bit_${LP3D_BUILD_TYPE}"
else
  BUILD_ARCH="32bit_${LP3D_BUILD_TYPE}"
fi
