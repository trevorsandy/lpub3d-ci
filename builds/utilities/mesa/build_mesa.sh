...
# specify the binary folder basename
lib_dir="${LIB_DIR:-lib}"
# tell curl to follow links and continue after abnormal end if supported
curl_opts="-L -C -"
# build log
log_path="${LOG_PATH:-$PWD}"
# specify if this is a RPM linux build (e.g. RedHat, Fedora, Suse etc...)
rpm_build="${RPM_BUILD:-}"
...
if [ ! -f "$mesa_prefix/${lib_dir}/lib${lib_file}.${lib_ext}" ]; then
  if [ "${use_autoconf}" -eq 1 ]; then
    ...
    $configure_options \
    --prefix=${mesa_prefix} \
    --libdir=${mesa_prefix}/${lib_dir} \
    "
    Info "Using confops: $configure_options" && Info

    # configure command
    env PKG_CONFIG_PATH="$mesa_prefix/${lib_dir}/pkgconfig:$PKG_CONFIG_PATH" \
    ...
    # configure command - libdir(${lib_dir}) is automatically detected based on the platform
    meson_options=(
    ...
    if [[ "${build_osmesa_only}" -ne 1 && "${static_build}" -ne 1 ]]; then
      ln -s $mesa_prefix/${lib_dir}/libGLX_amber.so.0 "$mesa_prefix/${lib_dir}/libGLX_indirect.so.0"
    fi
...
if  [ -f "${mesa_prefix}/osmesa-config" ]; then
  # set library folder basename
  sed -e "s;libdir=\${prefix}/.*;libdir=\${prefix}/${lib_dir};" -i ${mesa_prefix}/osmesa-config || :
  Info "osmesa-config libdir updated"
  sed -n '8p;9q' ${mesa_prefix}/osmesa-config
  # set version
...
--libdir=${mesa_prefix}/${lib_dir} \
"
if [ ! -f "${mesa_prefix}/${lib_dir}/libGLU.${lib_ext}" ]; then
  # configure command
  env PKG_CONFIG_PATH="${mesa_prefix}/${lib_dir}/pkgconfig:$PKG_CONFIG_PATH" \
  ./configure ${configure_options} CC="$CC" CFLAGS="$CFLAGS" CXX="$CXX" CXXFLAGS="$CXXFLAGS"
