...
function ShowHelp() {
    ...
    echo " - LPUB3D=lpub3d-ci - repository name"
    echo " - WORK_DIR=$HOME - work directory"
    ...
    echo " - BUILD_BRANCH=master - repository branch to be built"
...
function ShowOptions ()
{
    ...
    echo "SKIP_CLEANUP...[$([ -n "${SKIP_CLEANUP}" ] && echo "Yes" || echo "No")]"
    [ "$opt" = "Deb" ] && \
    echo "DEB_EXT........[${DEB_EXTENSION}]" || :
...
case $opt in
  Deb) buildFolder=$WORK_DIR/debbuild ;;
  Rpm) buildFolder=$WORK_DIR/rpmbuild ;;
  Pkg) buildFolder=$WORK_DIR/pkgbuild ;;
esac
    ...
    case $opt in
      ...
      if test -d "$buildFolder/SOURCES"; then
        ...
        ( cd "$buildFolder" && \
          ...
          mv SOURCES "$WORK_DIR" && rm -rf ./* && mv "$WORK_DIR/SOURCES" . && \
          for dir in $dirs; do if test -d "SOURCES/$dir"; then mv "SOURCES/$dir" .; fi; done && \
          cd "$WORK_DIR" && rm -rf ./*.log ./*assets.tar.gz ) >$l.out 2>&1 && rm $l.out
      fi
      ;;
      Rpm)
      if test -d "$buildFolder/SOURCES"; then
...
env LOCAL="${LOCAL}" DOCKER="${DOCKER}" LPUB3D="${LPUB3D}" PRESERVE="${PRESERVE}" \
UPDATE_SH="${UPDATE_SH}" LP3D_ARCH="${LP3D_ARCH}" BUILD_BRANCH="${BUILD_BRANCH}" \
XSERVER="${XSERVER}" DEB_EXTENSION="${DEB_EXT}" LOCAL_RESOURCE_PATH="${LOCAL_PATH}" "./${createScript}"
