...
if [ ! "$DEPLOY_LOCAL_LIBS" = 1 ]; then
  # Extact tarball
  cd $LP3D_LL_WD && [ ! -f "mesa-libOSMesa-17.0.1-6.20170307.el7.x86_64.rpm" ] && \
  ...
  ln -f -s $LP3D_LL_USR/bin/llvm-config-64 $LP3D_LL_USR/bin/llvm-config && \
  [ -f $LP3D_LL_USR/bin/llvm-config ] && \
  echo "Created symlink $LP3D_LL_USR/bin/llvm-config " || \
  echo "ERROR - unable to create symlink $LP3D_LL_USR/bin/llvm-config"

  # Export library bin paths
  echo "Export LP3D_LL_BIN: $LP3D_LL_BIN" && \
  export LP3D_LL_BIN || \
  echo "ERROR - Could not find Local Lib directory"
fi

# Export library base paths
[ -d $LP3D_LL_USR ] && \
echo "Export LP3D_LL_USR: $LP3D_LL_USR" && \
export LP3D_LL_USR || echo "ERROR - $LP3D_LL_USR does not exist"
[ -d $LP3D_LL_ETC ] && \
echo "Export LP3D_LL_ETC: $LP3D_LL_ETC" && \
export LP3D_LL_ETC || echo "ERROR - $LP3D_LL_ETC does not exist"
