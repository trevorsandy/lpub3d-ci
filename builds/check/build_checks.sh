...
for LP3D_BUILD_CHECK in CHECK01 CHECK02 CHECK03 CHECK04 CHECK05; do
    ...
    CHECK01)
        ...
        LP3D_CHECK_OPTIONS="--no-stdout-log --process-file --preferred-renderer native"
        ;;
    CHECK02)
        LP3D_CHECK_HDR="- Check 2 of 5: LDView File Process Check..."
        LP3D_CHECK_OPTIONS="--no-stdout-log --process-file --clear-cache --preferred-renderer ldview"
        ;;
    CHECK03)
        LP3D_CHECK_HDR="- Check 3 of 5: LDView (Single Call) File Process Check..."
        LP3D_CHECK_OPTIONS="--no-stdout-log --process-file --clear-cache --preferred-renderer ldview-sc"
        ;;
    CHECK04)
        LP3D_CHECK_HDR="- Check 4 of 5: LDGLite Export Range Check..."
        LP3D_CHECK_OPTIONS="--no-stdout-log --process-export --range 1-3 --clear-cache --preferred-renderer ldglite"
        ;;
    CHECK05)
        LP3D_CHECK_HDR="- Check 5 of 5: Native POV Generation Check..."
        LP3D_CHECK_OPTIONS="--no-stdout-log --process-file --clear-cache --preferred-renderer povray"
        ;;
      esac
