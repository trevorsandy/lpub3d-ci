...
if [ -n "$LDGLITE_EXE" ]; then
    test -f "$LDGLITE_EXE" && LDGLITE_SETTING="Available - $LDGLITE_EXE" || (LDGLITE_SETTING="Invalid - $LDGLITE_EXE" && unset LDGLITE_EXE)
else
    LDGLITE_SETTING=Invalid
fi
if [ -n "$LDVIEW_EXE" ]; then
    test -f "$LDVIEW_EXE" && LDVIEW_SETTING="Available - $LDVIEW_EXE" || (LDVIEW_SETTING="Invalid - $LDVIEW_EXE" && unset LDVIEW_EXE)
else
    LDVIEW_SETTING=Invalid
fi
if [ -n "$POVRAY_EXE" ]; then
    test -f "$POVRAY_EXE" && POVRAY_SETTING="Available - $POVRAY_EXE" || (POVRAY_SETTING="Invalid - $POVRAY_EXE" && unset POVRAY_EXE)
else
...
if [ "${LPUB3D_SETTING}" = "Invalid" ]; then
    if [ -n "${LPUB3D_EXE}" ]; then
        LPUB3D_INSERT=[${LPUB3D_EXE}] path is invalid
    else
        LPUB3D_INSERT=was not specified
        LPUB3D_SETTING=Undefined
    fi
    show_settings_and_exit "ERROR - LPub3D executable ${LPUB3D_INSERT}. Build check cannot be executed."
fi
...
if [[ "${LP3D_OS_NAME}" = "darwin" && "$BUILD_OPT" = "compile" ]]; then
    ...
    echo "- reset macOS LPub3D executable to $LPUB3D_EXE"
