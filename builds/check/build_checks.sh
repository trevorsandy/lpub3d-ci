...
function show_settings
{
    ...
    echo "--LPUB3D_EXE.........$([ -n "${LPUB3D_EXE}" ] && echo Available || echo Missing)"
    echo "--LP3D_LDGLITE.......$([ -n "${LDGLITE_EXE}" ] && echo Available || echo Missing)"
    echo "--LP3D_LDVIEW........$([ -n "${LDVIEW_EXE}" ] && echo Available || echo Missing)"
    echo "--LP3D_POVRAY........$([ -n "${POVRAY_EXE}" ] && echo Available || echo Missing)"
...
if [ -n "$LDGLITE_EXE" ]; then
    test -f "$LDGLITE_EXE" || unset LDGLITE_EXE
fi
if [ -n "$LDVIEW_EXE" ]; then
    test -f "$LDVIEW_EXE" || unset LDVIEW_EXE
fi
if [ -n "$POVRAY_EXE" ]; then
    test -f "$POVRAY_EXE" || unset POVRAY_EXE
fi
...
if [ -n "$LP3D_BUNDLED_APP" ]; then
    ...
    if [ -f "$LPUB3D_EXE" ]; then
        ...
        LP3D_EXE_INSERT="$([ -n "${LPUB3D_EXE}" ] && echo [${LPUB3D_EXE}] was not found || echo was not found)"
        show_settings_and_exit "ERROR - LPub3D executable ${LP3D_EXE_INSERT}. Build check cannot be executed."
    fi
fi
