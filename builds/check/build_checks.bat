...
IF NOT EXIST "%PKG_TARGET%" (
  ...
  ECHO.
  ECHO ----Build Check Status: PASS ^(%PKG_CHECK_PASS%^)%PKG_PASS_CHECKS%, FAIL ^(%PKG_CHECK_FAIL%^)%PKG_FAIL_CHECKS%----
  ECHO.
...
IF "!PKG_CHECK_TMP!" EQU "%PKG_CHECK_SUCCESS%" (
  ...
  SET "PKG_PASS_CHECKS=!PKG_PASS_CHECKS!,%2"
  FOR /F %%a IN ('SET PKG_CHECK_PASS^&SET PKG_PASS_CHECKS') DO (IF "!"=="" ENDLOCAL)&SET "%%a"
) ELSE (
  ECHO -%1 FAIL
  SET /A PKG_CHECK_FAIL=!PKG_CHECK_FAIL!+1
  SET "PKG_FAIL_CHECKS=!PKG_FAIL_CHECKS!,%2"
  FOR /F %%a IN ('SET PKG_CHECK_FAIL^&SET PKG_FAIL_CHECKS') DO (IF "!"=="" ENDLOCAL)&SET "%%a"
  TYPE "%PKG_LOG_FILE%"
)
DEL /Q "%PKG_LOG_FILE%"
EXIT /b
