...
IF %LP3D_AMD_UNIVERSAL_BUILD% NEQ 1 (
  ...
  IF !LP3D_DISTRO_ARCH!==x86 (
    ...
    CALL :DOWNLOADMSVCREDIST %LP3D_MSC32_VER% %LP3D_QT_MAJ_VER% !LP3D_DISTRO_ARCH!
    ...
    IF /I !LP3D_DISTRO_ARCH!==x86_64 CALL :DOWNLOADMSVCREDIST %LP3D_MSC64_VER% !LP3D_QT_MAJ_VER! !LP3D_DISTRO_ARCH!
    IF /I !LP3D_DISTRO_ARCH!==ARM64 CALL :DOWNLOADMSVCREDIST %LP3D_MSCARM64_VER% !LP3D_QT_MAJ_VER! !LP3D_DISTRO_ARCH!
  ...
  FOR %%A IN ( x86_64, x86 ) DO (
    ...
    IF %%A==x86 (
      ...
      CALL :SETSSLVERSION %LP3D_QT32VCVERSION%
      CALL :DOWNLOADOPENSSLLIBS
      CALL :DOWNLOADMSVCREDIST %LP3D_MSC32_VER% !LP3D_QT_MAJ_VER! %%A
    ) ELSE (
      rem %%A==x86_64
      ECHO   LP3D_MSC64_VER....................[%LP3D_MSC64_VER%]
      ECHO   LP3D_QT64_VCVERSION...............[%LP3D_QT64VCVERSION%]
      ...
      IF !WIN64_SSL! EQU 1 CALL :SETSSLVERSION %LP3D_QT64VCVERSION%
      IF !WIN64_SSL! EQU 1 CALL :DOWNLOADOPENSSLLIBS
      IF %%A==x86_64 CALL :DOWNLOADMSVCREDIST %LP3D_MSC64_VER% !LP3D_QT_MAJ_VER! %%A
    )
    CALL :COPYFILES
    ENDLOCAL DISABLEDELAYEDEXPANSION
  )
...
:DOWNLOADMSVCREDIST
rem MSVC redistributable versions - 1900 EQU 2015, 1910-16 EQU 2017, 1920-29 EQU 2019, 1930-44 EQU 2022
rem leave the year for 2019 blank
IF %1 GEQ 1900 (IF %1 LSS 1910 SET LP3D_MSVCR_VER=2015)
IF %1 GEQ 1910 (IF %1 LSS 1920 SET LP3D_MSVCR_VER=2017)
IF %1 GEQ 1920 (IF %1 LSS 1929 SET LP3D_MSVCR_VER=)
IF %1 GEQ 1930 SET LP3D_MSVCR_VER=2022
SET LP3D_QT_MAJ_VER=%2
SET LP3D_DISTRO_ARCH=%3
ECHO.
...
rem for Qt6 the redist folder is vcredist2022
SET "MSVC_REDIST_DIR=%DIST_DIR%\vcredist%LP3D_MSVCR_VER%"
...
IF "%LP3D_MSVCR_VER%" EQU "" (
  SET "RedistCONTENT=vcredist_%LP3D_DISTRO_ARCH%.exe"
) ELSE (
  IF %LP3D_QT_MAJ_VER% EQU 6 (
    IF "%LP3D_MSVCR_VER%" == "2022" (
      IF "%LP3D_DISTRO_ARCH%" == "ARM64" (
        SET RedistCONTENT=vc_redist_2022_arm64.exe
      ) ELSE (
        SET RedistCONTENT=vc_redist_2022_x64.exe
      )
    )
  ) ELSE (
    IF "%LP3D_MSVCR_VER%" == "2015" SET RedistCONTENT=vcredist_2015_%LP3D_DISTRO_ARCH%.exe
  )
)
...
IF "%LP3D_MSVCR_VER%" EQU "" (
  SET Redistributable=%RedistCONTENT%
) ELSE (
  IF %LP3D_QT_MAJ_VER% EQU 6 (
    IF "%LP3D_DISTRO_ARCH%" == "ARM64" (
      SET Redistributable=vc_redist.arm64.exe
    ) ELSE (
      SET Redistributable=vc_redist.x64.exe
    )
  ) ELSE (
    SET Redistributable=vcredist_%LP3D_DISTRO_ARCH%.exe
  )
)

IF NOT EXIST "%OutputPATH%\%Redistributable%" (
  CALL :CREATEWEBCONTENTDOWNLOADVBS
  CALL :GET_MSVC_REDIST %1 %LP3D_QT_MAJ_VER% %LP3D_MSVCR_VER%
) ELSE (
  ECHO.
  ECHO - MSVC %1 %LP3D_DISTRO_ARCH% Redistributable %Redistributable% exist. Nothing to do.
)

CALL :SET_MSVC_REDIST %1 %LP3D_QT_MAJ_VER% %LP3D_MSVCR_VER%
EXIT /b

:GET_MSVC_REDIST
SET LP3D_RENAME=0
SET LP3D_QT_MAJ_VER=%2
SET LP3D_MSVCR_VER=%3
...
IF "%LP3D_MSVCR_VER%" NEQ "" (
  IF %LP3D_QT_MAJ_VER% EQU 6 (
    IF "%LP3D_DISTRO_ARCH%" == "ARM64" (
      SET RedistCONTENT=vc_redist.arm64.exe
    ) ELSE (
      SET RedistCONTENT=vc_redist.x64.exe
    )
  ) ELSE (
    SET RedistCONTENT=vcredist_%LP3D_DISTRO_ARCH%.exe
  )
...
IF "%LP3D_MSVCR_VER%" EQU "" (
  ...
  IF %LP3D_QT_MAJ_VER% EQU 6 (
    IF "%LP3D_DISTRO_ARCH%" == "ARM64" (
      SET RedistTARGET=vc_redist.arm64.exe
    ) ELSE (
      SET RedistTARGET=vc_redist.x64.exe
    )
  ) ELSE (
...
IF NOT EXIST "%PKG_TARGET_DIR%\%RedistTARGET%" (
  ...
  IF EXIST "%MSVC_REDIST_DIR%\%RedistCONTENT%" (
    COPY /V /Y "%MSVC_REDIST_DIR%\%RedistCONTENT%" "%PKG_TARGET_DIR%\%RedistTARGET%" /B | FINDSTR /i /v /r /c:"copied\>"
  ) ELSE (
    ECHO.
    ECHO -ERROR - MSVC %1 %LP3D_DISTRO_ARCH% Redistributable %RedistCONTENT% does not exist in %MSVC_REDIST_DIR%\.
  )
) ELSE (
  ECHO.
