  ...
  QString &topLevelFile)
{
  if (modified) {
    ...
    if (marginsModified || marginsModified)
...
{
  modified = (unitsModified || valueModified);
  if (modified) {
    MetaItem mi;
    mi.setGlobalMeta(modelName,meta);
  }
...
{
  modified = (
    showSubmodelsModified ||
    showTopModelModified ||
    showSubmodelInCalloutModified ||
    showInstanceCountModified ||
    placementModified);
  if (modified) {
    applySettings();
    MetaItem mi;
...
void PliSortOrderGui::apply(QString &topLevelFile)
{
  modified = (
    ...
    secondaryModified ||
    tertiaryModified ||
    primaryDirectionModified ||
    secondaryDirectionModified ||
    tertiaryDirectionModified);
  if (modified) {
    MetaItem mi;
...
void PliPartElementGui::apply(QString &topLevelFile)
{
  modified = (
    displayModified ||
    bricklinkElementsModified ||
    legoElementsModified ||
    userElementsLDrawKeyModified ||
    userElementsModified);
  if (modified) {
    MetaItem mi;
    if (displayModified) {
      mi.setGlobalMeta(topLevelFile,&meta->display);
...
void PliAnnotationGui::apply(QString &topLevelFile)
{
  modified = (
    ...
    titleModified ||
    freeformModified ||
    titleAndFreeformModified ||
    fixedAnnotationsModified ||
    axleStyleModified ||
    beamStyleModified ||
    cableStyleModified ||
    connectorStyleModified ||
    elementStyleModified ||
    extendedStyleModified ||
    hoseStyleModified ||
    panelStyleModified);
  if (modified) {
    MetaItem mi;
...
void CsiAnnotationGui::apply(QString &topLevelFile)
{
  modified = (
    ...
    axleDisplayModified ||
    beamDisplayModified ||
    cableDisplayModified ||
    connectorDisplayModified ||
    extendedDisplayModified ||
    hoseDisplayModified ||
    panelDisplayModified ||
    placementModified);
  if (modified) {
    MetaItem mi;
    if (displayModified) {
...
void RotateIconSizeGui::valueWChange(const QString &string)
{
  ...
  dataWModified = metaModified(notEqual(value, dataH));
}
...
void RotateIconSizeGui::valueHChange(const QString &string)
{
  ...
  dataHModified = metaModified(notEqual(value, dataH));
}
...
{
  modified = (dataWModified || dataHModified);
  if (modified) {
    MetaItem mi;
    mi.setGlobalMeta(topLevelFile,meta);
  }
...
void SizeAndOrientationGui::apply(QString &topLevelFile)
{
  ...
  if (modified) {
    MetaItem mi;
    if (sizeIDModified || sizeModified || sizeModified) {
...
{
  modified = (
  subModelLevel0ColorModified ||
  subModelLevel1ColorModified ||
  subModelLevel2ColorModified ||
  subModelLevel3ColorModified);
  if (modified) {
    MetaItem mi;
    mi.setGlobalMeta(topLevelFile,meta);
  }
