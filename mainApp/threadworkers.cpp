...
void PartWorker::processCustomColourParts(PartType partType, bool overwrite, bool setup)
{
  ...
  //emit progressLabelPermSetTextSig("Parse Model File");
  Paths::mkCustomDirs();

  int subfiles = lpub->ldrawFile._subFileOrder.size();

  // process top-level submodels
  //emit progressBarPermSetRangeSig(1, subfiles);
  ...
  for (int i = 0; i < subfiles && endThreadNotRequested(); i++) {
      ...
      //emit progressBarPermSetValueSig(i);
...
//#endif
  ...
  //emit progressBarPermSetValueSig(ldrawFile._subFileOrder.size());
...
bool PartWorker::processColourParts(const QStringList &colourPartList, const PartType partType) {
    ...
    //DISABLE PROGRESS BAR - CAUSING MESSAGEBAR OVERLOAD
    //emit progressBarPermResetSig();
    //emit progressLabelPermSetTextSig("Process Color Parts...");
    //emit progressBarPermSetRangeSig(1, colourPartList.size());
    //int partCount = 0;
    ...
    for (QString const &partEntry : colourPartList) {
        ...
        //emit progressBarPermSetValueSig(partCount++);
...
//#endif
    ...
    //emit progressBarPermSetValueSig(colourPartList.size());
...
bool PartWorker::createCustomPartFiles(const PartType partType, bool  overwriteCustomParts) {
    ...
    //emit progressBarPermResetSig();
    //emit progressLabelPermSetTextSig("Creating Custom Color Parts");
    //emit progressBarPermSetRangeSig(1, maxValue);
    ...
    for(int part = 0; part < _partList.size() && endThreadNotRequested(); part++) {
        ...
        //emit progressBarPermSetValueSig(part);
    ...
    //emit progressBarPermSetValueSig(maxValue);
    return true;
}
...
bool PartWorker::processPartsArchive(const QStringList &ldPartsDirs, const QString &comment, bool overwriteCustomParts) {
  ...
  //if (okToEmitToProgressBar())
  //    emit progressBarPermSetRangeSig(0, 0);
  ...
  for (int i = 0; i < searchDirs && endThreadNotRequested(); i++) {
      ...
      emit progressLabelPermSetTextSig(tr("Archiving %1 parts...\nProcessing: %2")
                              .arg(comment, partDir.absolutePath()));

      if (searchDirs > 1)
          emit progressBarPermSetValueSig(ArchiveParts::ProcessedParts(partDir.absolutePath()));
  ...
  //emit progressLabelPermSetTextSig(returnMessage);
...
bool ColourPartListWorker::processArchiveParts(const QString &archiveFile) {
   ...
   QString library = Preferences::usingDefaultLibrary ? tr("Unofficial %1 Library").arg(lib)  : tr("%1 Custom Library").arg(lib);
    ...
    // get part count
    emit progressBarPermSetRangeSig(0, 0);
    emit progressLabelPermSetTextSig("Generating " + library + " Color Parts...");
    ...
    emit progressBarPermResetSig();
    emit progressBarPermSetRangeSig(1, partCount);
    partCount = 0;
...
void ColourPartListWorker::processChildren() {
    ...
    emit progressBarPermResetSig();
    emit progressLabelPermSetTextSig("Processing Child Color Parts...");
    emit progressBarPermSetRangeSig(1, _partList.size());
    ...
    emit progressBarPermSetValueSig(_partList.size());
...
void ColourPartListWorker::fileSectionHeader(const int &option, const QString &heading) {
    ...
    case FADESTEP_INTRO_HEADER:
        ...
        _ldrawStaticColourParts  << tr("# fade previous steps and highlight current step. Parts on this list are identified in the LDraw");
        _ldrawStaticColourParts  << tr("# library and copied to their respective custom directory. Copied files are modified as described");
        _ldrawStaticColourParts  << tr("# in the following lines.");
        _ldrawStaticColourParts  << QString();
        _ldrawStaticColourParts  << tr("# When fade previous steps is enabled, color codes are replaced with a custom code using the");
        _ldrawStaticColourParts  << tr("# LDConfig color code prefixed with the 3-digit user configurable prefix [%1].").arg(Preferences::fadeStepsColourPrefix);
        _ldrawStaticColourParts  << tr("# If using a global fade steps color, subpart color codes are replaced with the main material color");
        _ldrawStaticColourParts  << tr("# [16] and top-level colors are replaced with the fade color set in Preferences or meta command.");
        _ldrawStaticColourParts  << QString();
        _ldrawStaticColourParts  << tr("# When highlight current step is enabled, the edge color values are replaced with the highlight");
        _ldrawStaticColourParts  << tr("# color value set in Preferences or meta command. Additionally, color codes are replaced with a");
        _ldrawStaticColourParts  << tr("# custom code using the LDConfi color code prefixed with the 3-digit user configurable prefix [%1].").arg(Preferences::highlightStepColourPrefix);
        _ldrawStaticColourParts  << QString();
        _ldrawStaticColourParts  << tr("# For fade previous steps, generated static color part files are appended with '%1',").arg(FADE_SFX);
        _ldrawStaticColourParts  << tr("# for example, ...\\custom\\parts\\99499%1.dat").arg(FADE_SFX);
        _ldrawStaticColourParts  << QString();
        _ldrawStaticColourParts  << tr("# For highlight current step, generated static color part files are appended with '%1',").arg(HIGHLIGHT_SFX);
        ...
        _ldrawStaticColourParts  << QString();
        _ldrawStaticColourParts  << tr("# This file is automatically generated from the menu action at:");
        _ldrawStaticColourParts  << tr("#    Configuration=>Generate Static Color Parts List");
        _ldrawStaticColourParts  << tr("# However, it can also be edited manually from the menu action at:");
